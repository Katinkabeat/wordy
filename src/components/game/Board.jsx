...
export default function Board({ board, placements, lastMoveTiles = [], onCellClick, myTurn, cellSize = 36, isDark = false, layoutVersion = 1 }) {
  ...
  const letterSize = Math.max(8,  Math.round(cellSize * 0.48))
  const valueSize  = Math.max(5,  Math.round(cellSize * 0.26))
...
function BoardTile({ letter, isBlank, hue, isNew, isLastMove, isDark, onClick, letterSize, valueSize }) {
  ...
  const style = boardTileStyle(hue, age, isDark)
  ...
  return (
    ...
    >
      <div className="relative flex items-center justify-center w-full h-full">
        <span
          style={{ fontSize: letterSize, color: style.color }}
          className="font-display select-none leading-none"
        >
          {letter}
        </span>
        <span
          style={{ fontSize: valueSize, color: style.valColor }}
