...
- **Verified:** `npm run build` compiles clean (GamePage chunk incl. useGameMutations). Runtime MP game-over not driven (needs 2 players finishing a full match); the finish_game RPC is unchanged and the refresh-and-retry mechanism was proven live E2E in Yahdle (success + self-heal) this same session. Wordy's SW is push-only (no fetch/caches) so no cache bump.
